...
export default function CustomizeSite({ templateName, templateImage, userName, site }: CustomizeSiteProps) {
    ...
    //
    // Regra:
    // - se o Instagram trouxe um dado, usamos o dado importado;
    // - se não trouxe, preservamos o conteúdo padrão do template.
    // ============================================================
    ...
    React.useEffect(() => {
        ...
        try {
            ...
            // Compatível com:
            // { profile: {...} }
            // e com o formato antigo onde o perfil era salvo diretamente.
            const profile = parsed.profile ?? parsed

            const importedName = (profile.name || profile.fullName || '').trim()
            const importedUsername = (profile.username || '').replace(/^@/, '').trim()
            ...
            const displayName = importedName || importedUsername
            const firstName = displayName.split(/\s+/)[0] || 'Personal'
            ...
            // A URL do Instagram serve para o preview.
            // Não apagamos a imagem padrão quando o Instagram não trouxe foto.
            if (importedImage) {
            ...
            const normalizedBio = importedBio
                .normalize('NFD')
                .replace(/[\u0300-\u036f]/g, '')
                .toLowerCase()

            const hasConstancy = /constancia|consistencia/.test(normalizedBio)
            const hasPersonalized = /personalizad/.test(normalizedBio)
            const hasWeightLoss = /emagrec|perder peso|perda de peso/.test(normalizedBio)
            const hasHypertrophy = /hipertrof|massa muscular/.test(normalizedBio)
            const hasPerformance = /performance|desempenho/.test(normalizedBio)

            let focus = 'resultados consistentes'

            if (hasWeightLoss) {
                focus = 'emagrecimento com estratégia'
            } else if (hasHypertrophy) {
                focus = 'hipertrofia e evolução'
            } else if (hasPerformance) {
                focus = 'performance e evolução'
            } else if (hasConstancy) {
                focus = 'constância e evolução'
            }
            ...
            setData((current) => ({
                ...
                // Só personalizamos o Hero quando há informações do perfil.
                hero:
                    displayName || importedBio || importedImage
                        ? {
                              ...current.hero,
                              ...(displayName
                                  ? {
                                        titlePrimary: `${displayName.toUpperCase()} • PERSONAL TRAINER`,
                                    }
                                  : {}),
                              ...(importedBio
                                  ? {
                ...
                // Só substituímos métricas que realmente vieram do Instagram.
                // As demais continuam exatamente como estavam no template.
                ...
                // A bio dá contexto suficiente para personalizar os serviços.
                // Se não houver bio, preservamos integralmente os serviços padrão.
                services: importedBio
                    ? {
                          ...current.services,
                          ...
                          cards: [
                              {
                                  ...
                                  // Instagram não informa preço/link:
                                  // mantemos os valores que já existem no template.
                              },
                              {
                ...
                about:
                    displayName || importedBio || importedImage
                        ? {
                              ...
                              ...(displayName
                                  ? {
                                        title: `${displayName}: treino com estratégia e acompanhamento`,
                                    }
                                  : {}),
                              ...(importedBio
                                  ? {
                ...
                // Portanto NÃO alteramos testimonials.
                // Os depoimentos padrão do Template 1 permanecem.
                testimonials: current.testimonials,

                contact:
                    displayName || importedBio
                        ? {
                              ...current.contact,
                ...
                // O Instagram não fornece o telefone do WhatsApp.
                // Mantemos telefone/enabled e alteramos somente a mensagem
                // quando temos um nome para personalizar.
