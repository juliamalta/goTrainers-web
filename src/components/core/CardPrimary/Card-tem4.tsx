...
function CardTemplate4({ text, number, title, icon, iconBgColor }: CardProps) {
    return (
        <div className="group relative h-full min-h-[240px] overflow-hidden rounded-2xl border border-white/10 bg-white p-px transition-all duration-300 hover:-translate-y-2">
            ...
            <div className="relative flex h-full min-w-0 flex-col rounded-2xl bg-white p-5 sm:p-6 2xl:p-8">
                ...
                <div className="flex items-start justify-between gap-4">
                    {/* Número */}
                    <div
                        className="flex h-10 w-10 items-center justify-center rounded-2xl p-8 text-xl font-bold tracking-tight text-color-burgundy"
                        style={{ backgroundColor: iconBgColor ?? 'var(--ivory)' }}>
                        {number}
                    </div>
                </div>

                {/* Linha */}
                <div className="h-px w-full bg-white sm:mt-6">
                    <div className="h-px w-0 bg-color-burgundy transition-all duration-500 group-hover:w-1/3" />
                </div>

                {/* Conteúdo */}
                <div className="mt-5">
                    <h2 className="text-sm font-semibold leading-5 text-color-burgundy sm:text-base sm:leading-6 2xl:text-lg">
                        {title}
                    </h2>
